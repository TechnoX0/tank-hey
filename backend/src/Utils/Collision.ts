...
class Collision {
    ...
    constructor(
        ...
        if (Array.isArray(verticesOrRadius)) {
            ...
            throw new Error(
                "Invalid verticesOrRadius type. Expected Vector2D[] or number."
            );
        }
    }
    ...
    public collidesWith(other: Collision): boolean {
        if (
            ...
            other.collisionType === CollisionType.circle
        ) {
            return this.collideCircleWithCircle(other);
        } else if (
            this.collisionType === CollisionType.circle &&
            other.collisionType === CollisionType.polygon
        ) {
            return this.collideCircleWithPolygon(other);
        } else if (
            this.collisionType === CollisionType.polygon &&
            other.collisionType === CollisionType.circle
        ) {
    ...
    public collideCircleWithPolygon(other: Collision): boolean {
        ...
        for (let i = 0; i < worldVertices.length; i++) {
            ...
            const closestPointSegment = this.closestPointOnSegment(
                this.position,
                v1,
                v2
            );
    ...
    private closestPointOnSegment(
        ...
        const t = Math.max(
            ...
            Math.min(
                1,
                point.subtract(segmentStart).dot(segment) / segmentLengthSq
            )
        );
